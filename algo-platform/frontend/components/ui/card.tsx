...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "glass rounded-xl border border-border/80 bg-panel/85 backdrop-blur transition-all duration-150 ease-out motion-safe:hover:-translate-y-0.5 motion-safe:hover:shadow-glow",
        className
      )}
      {...props}
    />
